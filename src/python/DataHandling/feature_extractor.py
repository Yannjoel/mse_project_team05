...
class Features:
    ...
    def get_features(self):
        ...
        for column in self.data.columns:
            vectorizer = TfidfVectorizer().fit(self.data[column])

            features[column + '_idf'] = self.idf(self.data[column], vectorizer)
            features[column + '_vsm'] = self.vector_space_model(self.data[column], vectorizer)
            features[column + '_bm25'] = self.bm25(self.data[column])
